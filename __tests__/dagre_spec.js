...
SvgRenderer = class SvgRenderer {
  ...
  icon(v, label, x, y) {
    var border_width, className, height, href, icon_height, icon_width, key, label_height, radius, rx, ry, width;
  ...
  cover(vos) {
    var border_width, height, icon_width, width, x, xmax, xmin, y, ymax, ymin;
    ({border_width, icon_width} = this.options.style);
    if (!vos.length) {
      vos.push({
        x: border_width,
        y: border_width,
    ...
    movespace: function() {
      ...
      finish = ({pageX, pageY, target}) => {
        ...
        if (this.move.id) {
          ...
          this.move.dx = this.zoom * (pageX - px);
          this.move.dy = this.zoom * (pageY - py);
          return this.reparse(this.move.id);
        }
      };
      return cb = {
        touchend: (e) => {
...
stringify = function (tokens, data) {
  ...
  for (i = 0, len = tokens.length; i < len; i++) {
    ...
    switch (type) {
      case 'nodes':
        dest += all.replace(syntax.pick_node, function ($, x, y, v) {
          var o;
          if (o = data.nodes[v]) {
            ({ x, y } = o);
            return `<${x} ${y}>${v}`;
